...
I will now repeat the analysis for the donor experiment - latency, kinship experiment - duration, and kinship experiment - latency. However I will provide less explanation since the workflow through the analysis is very similar. 

## Past donor experiment analysis - latency.

```{r}
mean.duration<-toString(mean(donors$latency))
...
p<-p+theme_bw()
p<-p+stat_summary(fun.y=mean, color="darkred", geom="point", size=5)
p<-p+annotate("text", x=1, y=-140, label=mean.duration)
p  
