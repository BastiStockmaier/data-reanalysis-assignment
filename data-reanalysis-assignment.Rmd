...
```
The authors show a 95% CI for this mean of 12-91 seconds in their paper. This comes close to our calculation using the default option for calculating CI's in the package "boot" which uses a normal approximation. Since bootstrapping is a random process these results will slightly change every time the function is executed. This can be prevented by using the set.seed(123) function. 
